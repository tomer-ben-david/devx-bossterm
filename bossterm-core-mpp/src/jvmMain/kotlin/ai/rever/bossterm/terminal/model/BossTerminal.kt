...
class BossTerminal(
  ...
  private val myWindowTitlesStack = Stack<String>()
     ...
     * Buffer columns include DWC markers, surrogate pairs, and other multi-code-point sequences.
     * Visual columns represent what the user sees on screen.
     */
    private fun bufferColToVisualCol(bufferCol: Int): Int {
        if (bufferCol <= 0) return 0

        val line = try {
            terminalTextBuffer.getLine(myCursorY - 1)
        } catch (e: Exception) {
            return bufferCol  // Fallback if line not accessible
        }

        var visualCol = 0
        var col = 0
        val width = myTerminalWidth
        ...
        while (col < bufferCol && col < width) {
            ...
            // Skip low surrogates (part of previous high surrogate)
            if (Character.isLowSurrogate(char)) {
                col++
                continue
            }

            // Skip skin tone modifiers (U+1F3FB-U+1F3FF as surrogate pairs)
            ...
            // Regular character - count visual width (1 or 2)
     ...
     * Used when receiving cursor position commands from shell (which use visual columns).
     */
    private fun visualColToBufferCol(visualCol: Int): Int {
        if (visualCol <= 0) return 0

        val line = try {
            terminalTextBuffer.getLine(myCursorY - 1)
        } catch (e: Exception) {
            return visualCol  // Fallback if line not accessible
        }
        ...
        while (col < width && currentVisualCol < visualCol) {
            ...
            currentVisualCol += getCharacterVisualWidth(line, col, width)
            col++
        }
        return col
    }
    ...
    private fun getCharacterVisualWidth(line: TerminalLine, col: Int, width: Int): Int {
        ...
        // Simple case: next char is DWC (single BMP double-width char)
        if (col + 1 < width && line.charAt(col + 1) == CharUtils.DWC) {
            return 2
        }

        // For surrogate pairs, scan forward through extenders to find DWC
